...
def identity_block(input_tensor, kernel_size, filters, stage, block):
    ...
    x = Conv3D(filters2, kernel_size, padding='same', name=conv_name_base + '2b')(x)
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2, 2)):
    ...
    x = Conv3D(filters1, (1, 1, 1), strides=strides, name=conv_name_base + '2a')(input_tensor)
    x = BatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)
    ...
    shortcut = Conv3D(filters3, (1, 1, 1), strides=strides, name=conv_name_base + '1')(input_tensor)
...
def up_conv_block(input_tensor, kernel_size, filters, stage, block, strides=(1, 1, 1)):
    ...
    x = UpSampling3D(size=(2, 2, 2), name=up_conv_name_base + '2a')(input_tensor)

    x = Conv3D(filters1, (1, 1, 1), strides=strides, name=conv_name_base + '2a')(x)
    ...
    shortcut = UpSampling3D(size=(2, 2, 2), name=up_conv_name_base + '1')(input_tensor)    
    shortcut = Conv3D(filters3, (1, 1, 1), strides=strides, name=conv_name_base + '1')(shortcut)
    shortcut = BatchNormalization(axis=bn_axis, name=bn_name_base + '1')(shortcut)
...
def voxResnetModel(input_tensor=None, input_shape=None, classes=1):
    ...
    if input_shape is None:
        img_input = Input(shape=(224, 224, 96, classes))
    x = ZeroPadding3D((4, 4, 4))(img_input)
    ...
    x = up_conv_block(x, 3, [256, 64, 64], stage=10, block='a', strides=(1, 1, 1))
